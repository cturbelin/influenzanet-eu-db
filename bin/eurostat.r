...
geo.levels = nuts2
geo.levels = rename(geo.levels, list("nuts.code"="code_nuts2", "country.code"="country"))
geo.levels$code_nuts2 = as.character(geo.levels$code_nuts2)
geo.levels$code_nuts1 = substr(geo.levels$code_nuts2, 1, 3)

write.csv2(geo.levels, file="data/nuts/geo_levels.csv", row.names=F)

for(i in 1:3) {
  g = nuts[ nuts$nuts.level == i, c('nuts.code','country.code', 'nuts.label')]
  if(i > 1) {
    g$upper = substr(g$nuts.code, 1, (i-1) + 2)
    n = list(paste("code_nuts", i-1, sep=''))
    names(n) <- 'upper'
    g = rename(g, n)
  }
  g = rename(g, list("nuts.code"=paste("code_nuts",i, sep=''), 'country.code'="country", 'nuts.label'="title"))
  write.csv2(g, file=paste("data/nuts/geo_nuts",i,".csv",sep=''),  row.names=F)
}
...
for(year in output.years) {  
  ...
  pop = pop[, c('geo','country.code','all','male','female','flags','year.ref', 'age.min','age.max')]
  ...
  write.csv(pop, file=paste("data/population/",year,"_pop_nuts2_age5.csv",sep=''), row.names=F)
  ...
  p = aggregate(cbind(all, male, female) ~ code_nuts1 + age.min + age.max, data=pop, sum)
  ...
  p = aggregate(cbind(all, male, female) ~ country + age.min + age.max, data=pop.nuts1, sum)
